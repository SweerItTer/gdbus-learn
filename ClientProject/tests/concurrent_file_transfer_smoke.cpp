...
int main() {
    try {
        const fs::path server_dir = fs::path(TRAINING_SERVER_DIR);
        ...
        fs::remove(server_dir / source_a.filename());
        fs::remove(server_dir / source_b.filename());

        auto future_a = std::async(std::launch::async, RunSendFile, source_a);
        auto future_b = std::async(std::launch::async, RunSendFile, source_b);
        ...
        if (ReadFile(server_dir / source_a.filename()) != payload_a ||
            ReadFile(server_dir / source_b.filename()) != payload_b) {
            std::cerr << "concurrent different-name target mismatch" << std::endl;
            return 1;
        }

        // 同名同MD5并发发送：后来者直接拒绝，只保留一个
        ...
        fs::remove(server_dir / "same_ok.bin");

        auto future_same_ok_a = std::async(std::launch::async, RunSendFile, same_ok_a);
        auto future_same_ok_b = std::async(std::launch::async, RunSendFile, same_ok_b);
        ...
        if (!((same_ok_status_a == 0 && same_ok_status_b != 0) ||
              (same_ok_status_a != 0 && same_ok_status_b == 0))) {
            std::cerr << "same-name same-md5 should keep exactly one successful transfer" << std::endl;
            return 1;
        }

        if (!fs::exists(server_dir / "same_ok.bin") ||
            ReadFile(server_dir / "same_ok.bin") != same_ok_payload) {
            std::cerr << "same-name same-md5 final file mismatch" << std::endl;
            return 1;
        }

        // 同名不同MD5并发发送：后来者直接拒绝，最终只保留一个结果文件
        ...
        fs::remove(server_dir / "same_name.bin");

        auto future_same_a = std::async(std::launch::async, RunSendFile, same_a);
        auto future_same_b = std::async(std::launch::async, RunSendFile, same_b);
        ...
        if (!fs::exists(server_dir / "same_name.bin")) {
